...
## Status de implementação (aiDeck runtime)

Implementado em `feat/ds-v2.1-widgets` — ver `docs/decisions.md` (2026-06-16, "DS v2.1 widget extension").

| Item | Widget key | Componente | Notas |
|------|-----------|------------|-------|
| 1 | `stepper` | `StepperWidget.vue` | **reassigned** — antes era alias de `phase-timeline`; agora é o widget compacto. `phase-timeline` mantém os cards explodidos. |
| 2 | `status-list` | `StatusListWidget.vue` | `variant: checklist` para critérios. |
| 3 | `callout` | `CalloutWidget.vue` | enhanced — `eyebrow` + `tone` direto (inclui `neutral`); `variant` continua. |
| 4 | `collection-grid` | `CollectionGridWidget.vue` | corpo do card via slots (`header`/`body`/`footer`); `attention`/`live` por registro. |
| 5 | `record-switcher` | `RecordSwitcherWidget.vue` | seleção = navegação de rota; "current" casa `route.path`. |
| 6 | `catalog` | `CatalogWidget.vue` | single-layout, full-height; refs formam o grafo. |
| 7 | `headline-banner` | `HeadlineBannerWidget.vue` | `count` cai para `source.length`. |
| enh | `progress-bar` | `ProgressBarWidget.vue` | `tone` · `valueText` · `caption` · `segmented`. |
| enh | `card` | `CardWidget.vue` | já era host de slots (sem código novo). |
| **deferred** | `header-nav` `?`-help | — | a ação de ajuda do chrome toca o shell; follow-up dedicado. |
